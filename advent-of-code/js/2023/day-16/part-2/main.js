...
for (let r = 0; r < rows; r++) {
  maxEnergized = Math.max(
    maxEnergized,
    getEnergized({ row: r, col: 0, direction: "RIGHT" }, contraption).length,
    getEnergized({ row: r, col: cols - 1, direction: "LEFT" }, contraption).length,
  );
}
for (let c = 0; c < cols; c++) {
  maxEnergized = Math.max(
    maxEnergized,
    getEnergized({ row: 0, col: c, direction: "DOWN" }, contraption).length,
    getEnergized({ row: rows - 1, col: c, direction: "UP" }, contraption).length,
  );
}

console.log("Result:", maxEnergized);
